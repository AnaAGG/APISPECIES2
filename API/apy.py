...
from endpoints import species_, insert_data,  species_comun, species_month, delete_data, update_data , get_close, query_map, get_read_df, map_
from flask_pymongo import PyMongo
from functions import create_dict_insert, get_genus, get_date, get_coord, extract_info_form, extract_info_delete, extract_info_update, create_dict_update, extract_info_geo, create_dict_coord, get_info_class
...
@app.route("/province", methods = ["POST", "GET"]) #me devuelve las especies para determinada localidad
def data():    
...
def species_province_test():
    if request.method == "POST":
        ...
        return json_util.dumps(species_comun(province, sp))
    return render_template("read_species_province.html")

@app.route("/province/month", methods = ["POST", "GET"]) #me devuelve las especies para determinada localidad
def species_month_test():
    if request.method == "POST":
        province = request.form.get("province")
        month = request.form.get("month")
        return json_util.dumps(species_month(province, month))
